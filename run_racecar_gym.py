...
def list_available_scenarios():
    ...
    import racecar_gym.racecar_gym.envs.gym_api
...
def run_episode(
    env, policy_fn=random_policy, max_steps=1000, render=True, reset_mode="grid"
):
    """
...
class KeyboardController:
    ...
    def __init__(self):
        ...
        print("\n  Q: Quit episode")
        print(
            "  (Press and hold keys simultaneously for combinations, no need to press Enter)\n"
        )
    ...
    def update(self):
        ...
        if keys or len(self.active_keys) > 1:
            active_str = (
                ", ".join(sorted(self.active_keys)) if self.active_keys else "none"
            )
            combo_note = ""
            if len(self.active_keys) > 1:
                combo_note = " [COMBO]"
            print(
                f"  [Keys: {active_str}{combo_note}] Steering: {self.steering:.2f}, Motor: {self.motor:.2f}"
            )
...
def diagnose_environments():
    ...
    try:
        ...
        racecar_envs = [
            env for env in all_envs if "race" in env.lower() or "car" in env.lower()
        ]
    ...
    for scenario in test_scenarios:
        ...
        methods = [
            ...
            (f"gym.make('{scenario}')", lambda: gym.make(scenario)),
            (f"racecar_gym.make('{scenario}')", lambda: racecar_gym.make(scenario)),
        ]
